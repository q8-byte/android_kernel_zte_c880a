...
#ifdef CONFIG_OF_RESERVED_MEM
int dram_dummy_read_reserve_mem_of_init(struct reserved_mem *rmem, unsigned long node, const char *uname)
{
	phys_addr_t rptr = 0;
	...
	if (strcmp(uname, DRAM_R0_DUMMY_READ_RESERVED_KEY) == 0) {
		dram_base = rptr;
		dram_rank_num++;
		pr_debug("[dram_dummy_read_reserve_mem_of_init] dram_base = %pa, size = 0x%x\n", &dram_base, rsize);
	}

	if (strcmp(uname, DRAM_R1_DUMMY_READ_RESERVED_KEY) == 0) {
...
RESERVEDMEM_OF_DECLARE(dram_reserve_r0_dummy_read_init, DRAM_R0_DUMMY_READ_RESERVED_KEY, dram_dummy_read_reserve_mem_of_init);
RESERVEDMEM_OF_DECLARE(dram_reserve_r1_dummy_read_init, DRAM_R1_DUMMY_READ_RESERVED_KEY, dram_dummy_read_reserve_mem_of_init);
#endif
