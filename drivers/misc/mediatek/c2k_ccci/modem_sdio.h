...
#define C2K_IOCTL_PREPARE_BL		_IOW('c', 0, char[64])
#define C2K_IOCTL_PREPARE_MD_IMG	_IOW('c', 1, char[64])
#define C2K_IOCTL_GET_BL_SIZE		_IOR('c', 2, int)
#define C2K_IOCTL_GET_MD_IMG_SIZE	_IOR('c', 3, int)
#define C2K_IOCTL_DOWN_IMG_DONE		_IO('c', 4)
#define C2K_IOCTL_IS_IMG_BUF_RESERVED _IOR('c', 5, int)

#define IMG_LEN_MAX (4*1024*1024)
...
struct sdio_modem{
	...
	struct ccci_image_info bl_img_info;
	unsigned char *c2k_img_buf;
...
extern void dump_c2k_bootup_status();
