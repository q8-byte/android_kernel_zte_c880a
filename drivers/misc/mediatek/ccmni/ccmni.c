...
static int ccmni_open(struct net_device *dev)
{
	...
	CCMNI_INF_MSG(ccmni->md_id, "%s_Open: cnt=(%d,%d), md_ab=0x%X\n", \
		dev->name, atomic_read(&ccmni->usage), atomic_read(&ccmni_tmp->usage), ccmni_ctl->ccci_ops->md_ability);
	return 0;
}
...
static int ccmni_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCSTXQSTATE:
		...
		case SIOCCCMNICFG:
			...
			CCMNI_INF_MSG(md_id, "SIOCCCMNICFG: %s iRAT MD%d->MD%d, dev_cnt=%d, md_cnt=%d, md_irat_cnt=%d\n", \
				dev->name, (md_id+1), (ifr->ifr_ifru.ifru_ivalue+1), atomic_read(&ccmni->usage), atomic_read(&ccmni_tmp->usage), atomic_read(&ccmni_irat->usage));
			break;
	...
	switch(state) {
		...
		case TX_IRQ:
			if(netif_running(ccmni->dev) && netif_queue_stopped(ccmni->dev) && atomic_read(&ccmni->usage)>0) {
				...
				CCMNI_INF_MSG(md_id, "md_state_cb: %s, md_sta=TX_IRQ, usage=%d\n", \
