...
{
       APS_LOG("epl2182_report_interrupt_data:%d(0->3, 1->100)\n", value);
	if (value==0)
		return ps_report_interrupt_data(3);
	else if (value==1)
		return ps_report_interrupt_data(100);
}
...
static int epl2182_get_als_value(struct epl2182_priv *obj, u16 als)
{
   ...
   // }
	...
	wake_lock(&epl2182_ps_wake_lock);
        als_value_backup = obj->hw->als_value[idx];
...
/*----------------------------------------------------------------------------*/
int epl2182_get_addr(struct epl_alsps_hw *hw, struct epl2182_i2c_addr *addr)
{
    if(!hw || !addr)
    {
        return -EFAULT;
...
/*----------------------------------------------------------------------------*/
static void epl2182_power(struct epl_alsps_hw *hw, unsigned int on)
{
...
int epl2182_setup_eint(struct i2c_client *client)
{
	...
	irq_set_irq_wake(GPIO_ALS_EINT_PIN, 1);
...
static int epl2182_init_client(struct i2c_client *client)
{
    ...
    APS_LOG("epl2182_init_client, I2C Addr==[0x%x],line=%d\n",epl2182_i2c_client->addr,__LINE__);
...
{
	epl2182_proc_file = proc_create("driver/tsl2771_threshold", 0666, NULL, &epl2182_proc_fops);
