...
#define FPC_DEBUG  mdelay(10);printk("%s\n", __func__)

//#define FPC1020_IRQ_GPIO        (GPIO130 )
//#define FPC1020_RESET_GPIO      (GPIO129)


#define FPC1020_INT_IRQNO       CUST_EINT_FPC_NUM
#define FPC1020_IRQ_GPIO        GPIO_FPC_EINT_PIN
#define FPC1020_RESET_GPIO      GPIO_FPC_RST_PIN
#define FPC1020_EN_GPIO      GPIO_FPC_EN_PIN

extern int fp_sensor_type;
...
static u8 fpc1020_finger_up_reset(fpc1020_data_t *fpc1020);
...
//add by tony 
static int fpc1020_detect_finger_up_down(fpc1020_data_t *fpc1020);
...
static struct spi_board_info spi_board_devs[] __initdata = {
	[0] = {
        .modalias=FPC1020_SPI_DEV_NAME,  //   "fpc1020",
		.bus_num = 0,
		.chip_select=0,
		.mode = SPI_MODE_0,
	},
...
#define DEVFS_SETUP_MODE (S_IWUSR|S_IWGRP|S_IRUSR|S_IRGRP|S_IROTH)    //for cts
...
static FPC1020_DEV_ATTR(diag, spi_test ,	DEVFS_DIAG_MODE_RW);
...
static FPC1020_DEV_ATTR(diag, finger_up, DEVFS_DIAG_MODE_RO);
static FPC1020_DEV_ATTR(diag, finger_up_down, DEVFS_DIAG_MODE_RO);//add for pv fastmmi
static FPC1020_DEV_ATTR(diag, switch_to_input, DEVFS_DIAG_MODE_RO);//add for pv fastmmi
static FPC1020_DEV_ATTR(diag, go_to_deep_sleep, DEVFS_DIAG_MODE_RO);
...
static struct attribute *fpc1020_diag_attrs[] = {
	...
	&fpc1020_attr_spi_test.attr.attr,
	&fpc1020_attr_last_capture_time.attr.attr,
	&fpc1020_attr_finger_present_status.attr.attr,
	&fpc1020_attr_finger_up.attr.attr,
	&fpc1020_attr_finger_up_down.attr.attr,//add for pv fastmmi
	&fpc1020_attr_switch_to_input.attr.attr,
	&fpc1020_attr_go_to_deep_sleep.attr.attr,
	NULL
};
...
static int __init fpc1020_init(void)
{
	...
	if(fp_sensor_type!=0)
	{
        printk( "fp sensor is ok(%d),not fpc1020\n", fp_sensor_type);
		return 0;
	}
	gfx1xm_quit();
...
/* -------------------------------------------------------------------- */
void fpc1020_Power_OnOff(bool enable)
{
	if(enable)
	{
	       mt_set_gpio_mode(FPC1020_EN_GPIO, GPIO_MODE_00);
       	mt_set_gpio_dir(FPC1020_EN_GPIO, GPIO_DIR_OUT);
      		mt_set_gpio_out(FPC1020_EN_GPIO, GPIO_OUT_ONE);
       	mdelay(10);
	}
	else
	{
		//mt_set_gpio_mode(FPC1020_EN_GPIO, GPIO_MODE_00);
       	//mt_set_gpio_dir(FPC1020_EN_GPIO, GPIO_DIR_OUT);
      		mt_set_gpio_out(FPC1020_EN_GPIO, GPIO_OUT_ZERO);
       	//mdelay(10);
	}
}
...
//   for LDO setup	
	...
	mutex_init(&fpc1020->fpc_mutex_lock);//add by tony
	...
	//fpc1020_pdata->cs_gpio    = FPC1020_CS_GPIO;
	fpc1020_pdata->force_hwid = 0;
...
*/
	...
	wake_lock_init(&fpc1020->fpc_wake_lock, WAKE_LOCK_SUSPEND, "fpc irq wakelock");
...
/*	
#ifdef CONFIG_HAS_EARLYSUSPEND
	fpc1020->early_drv.level    = EARLY_SUSPEND_LEVEL_DISABLE_FB - 1,
	fpc1020->early_drv.suspend  = fpc1020_early_suspend,
	fpc1020->early_drv.resume   = fpc1020_late_resume,    
	register_early_suspend(&fpc1020->early_drv);
#endif
*/
	fp_sensor_type=2;
...
static int fpc1020_suspend(struct device *dev)
{
	...
	//enable_irq_wake(FPC1020_INT_IRQNO);
...
static int fpc1020_resume(struct device *dev)
{
	...
	dev_err(&fpc1020->spi->dev, "%s\n", __func__);
	//disable_irq_wake(FPC1020_INT_IRQNO);
/*
...
static void fpc1020_early_suspend(struct early_suspend *h) 
{  
	fpc1020_data_t *fpc1020 = container_of(h, fpc1020_data_t, early_drv); 
...
static unsigned int fpc1020_poll(struct file *file, poll_table *wait)
{
	...
	if (fpc1020->capture.available_bytes > 0)
		...
		blocking_op =
			((mode == FPC1020_MODE_WAIT_AND_CAPTURE) || (mode == FPC1020_CAPTURE_STATE_WAIT_FOR_FINGER_DOWN) || (mode == FPC1020_CAPTURE_STATE_WAIT_FOR_FINGER_UP));
...
irqreturn_t fpc1020_interrupt(void)
{
	...
	if (mt_get_gpio_in(fpc1020->irq_gpio)) {
		...
		printk("----%s--2   uevent_enable=%d\n",__func__,fpc1020->uevent_enable);
					...
					char *buf)
{
	...
	case offsetof(fpc1020_diag_t, spi_data):
		...
		break;
	case offsetof(fpc1020_diag_t, spi_test):
		fpc1020->diag.spi_register=96;
		fpc1020->diag.spi_regsize=1;
		//is_buffer = (fpc1020->diag.spi_regsize > sizeof(val));
		FPC1020_MK_REG_READ(reg,fpc1020->diag.spi_register, &spi_test_buf);
		fpc1020_reg_access(fpc1020, &reg);
		//error=0;
		//val=spi_test_buf;
		printk("tony:read %d\n",spi_test_buf);
	...
	case offsetof(fpc1020_diag_t, finger_present_status):
		...
		break;
	case offsetof(fpc1020_diag_t, finger_up):
		if(fpc1020_finger_up_reset(fpc1020) == 0)
		{
			fpc1020_capture_settings(fpc1020, FPC1020_MAX_ADC_SETTINGS - 1);
			error = fpc1020_check_finger_present_raw(fpc1020);
			dev_err(&fpc1020->spi->dev, "error=%d\n",error);
			if(error >= 64)
				val = 1;
			else if(error >= 0)
				val =0;
		...
		error = 0;
		break;
	case offsetof(fpc1020_diag_t, finger_up_down):

		error=fpc1020_detect_finger_up_down(fpc1020);
		if(error>=0)
			val= error ? 1:0;
		else
			val= 2;
		error = 0;
		break;
	case offsetof(fpc1020_diag_t, switch_to_input):
		fpc1020_start_input(fpc1020);
		error = 0;
		val = 0;
		break;
	case offsetof(fpc1020_diag_t, go_to_deep_sleep):
		printk("come in:go to deep sleep \n");
					...
					size_t count)
{
	...
	else if (fpc_attr->offset == offsetof(fpc1020_diag_t, spi_test)) {
			//error = kstrtou64(buf, 0, &val);
			val = buf[0];
			printk("tony:write %d\n",buf[0]);
			fpc1020->diag.spi_register=96;
...
static u8 fpc1020_finger_up_reset(fpc1020_data_t *fpc1020)
{
	...
	{	
		...
		if (error) {
			dev_err(&fpc1020->spi->dev, " fpc1020_finger_up_reset failed.\n");
		}
		else{
			error = 0;
...
static int fpc1020_detect_finger_up_down(fpc1020_data_t *fpc1020)
{
	...
	int error = 0;
	fpc1020_selftest_short(fpc1020);
	fpc1020_capture_settings(fpc1020, FPC1020_MAX_ADC_SETTINGS - 1);
       error = fpc1020_check_finger_present_raw(fpc1020);
	dev_err(&fpc1020->spi->dev, "%s %d\n", __func__, error);
       if(error>=100)
		...
		return -1;//don`t know,reset please
}
