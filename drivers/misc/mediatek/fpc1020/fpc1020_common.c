...
static const struct chip_struct chip_data[] = {
	...
	{FPC1020_CHIP_1022X, 0x0111, 1, FPC1022_ROWS, FPC1022_COLUMNS, FPC102X_ADC_GROUP_SIZE, 5000, 1000},
	{FPC1020_CHIP_1035X, 0x8110, 1, FPC1022_ROWS, FPC1022_COLUMNS, FPC102X_ADC_GROUP_SIZE, 5000, 1000},
...
void fpc1020_gpio_init(void)
{
	...
	/*irq*/
	mt_set_gpio_mode(GPIO_FPC_EINT_PIN, GPIO_FPC_EINT_PIN_M_EINT);	
  	mt_set_gpio_dir(GPIO_FPC_EINT_PIN, GPIO_DIR_IN);	
	mt_set_gpio_pull_enable(GPIO_FPC_EINT_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_FPC_EINT_PIN, GPIO_PULL_DOWN);
	//mt_set_gpio_pull_select(GPIO_FPC_EINT_PIN, GPIO_PULL_UP);	
...
//#ifdef MTK_PLATFORM
		...
		printk("%s  level=%d  gpio=%08x\n",__func__,mt_get_gpio_in(fpc1020->irq_gpio) ,fpc1020->irq_gpio);
#else
		      ...
		      fpc1020_reg_access_t *reg_data)
{
	...
	//printk("%s  level=%d  gpio=%08x\n",__func__,mt_get_gpio_in(fpc1020->cs_gpio) ,fpc1020->cs_gpio);
				...
				size_t buff_size)
{
	...
	if (!error) {
		...
		dev_err(&fpc1020->spi->dev, "length is %d  rest_length is %d\n",length ,rest_length);
		struct spi_transfer cmd = {
			.cs_change = 0,
			.delay_usecs = 0,
			.speed_hz = FPC1020_SPI_CLOCK_SPEED,
		...
		if( rest_length > 0) {
			...
			spi_message_add_tail(&cmd2, &msg2); //Note: if this does not work, just do it for a new spi_sync()
			error = spi_sync(fpc1020->spi, &msg2);
