...
#define GPIO_LIS2DS12_EINT_PIN 			GPIO_ALS_EINT_PIN			//eint gpio pin num
#define GPIO_LIS2DS12_EINT_PIN_M_EINT 	GPIO_ALS_EINT_PIN_M_EINT	//eint mode
#define CUST_EINT_LIS2DS12_NUM 			CUST_EINT_ALS_NUM			//eint num
#define CUST_EINT_LIS2DS12_DEBOUNCE_CN 	CUST_EINT_ALS_DEBOUNCE_CN 	//debounce time
#define CUST_EINT_LIS2DS12_TYPE 		CUST_EINT_ALS_TYPE			//eint trigger type
...
static int LIS2DS12_Write_PedoParameters(struct i2c_client *client, u8 pedo4g, u8 threshold, u8 debounce)
{
	...
	databuf[0] &= ~LIS2DS12_STEP_CNT_DEBOUNCE_MASK; // clear
...
static int LIS2DS12_enable_pedo(struct i2c_client *client, bool enable)
{
	...
	#if 0
		res = LIS2DS12_Write_PedoParameters(client, 1, 0x10, 0xff);// set threshold to a certain value here
		if(res != LIS2DS12_SUCCESS)
		{
...
static int LIS2DS12_InitClient(struct i2c_client *client, bool enable)
{
	...
	res = LIS2DS12_SetFullScale(client, LIS2DS12_ACC_RANGE_2g);//we have only this choice
	if(res != LIS2DS12_SUCCESS) 
	{
		return res;
	}
