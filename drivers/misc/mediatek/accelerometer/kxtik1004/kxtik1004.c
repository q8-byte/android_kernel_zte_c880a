...
static ssize_t store_selftest_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{ 
//huangzhaosong@wind-mobi.com 201500721 s--	
		GSE_ERR("invalid content: '%s', length = %d\n", buf, (int)count);   
//huangzhaosong@wind-mobi.com 201500721 e--
...
static int kxtik1004_local_init(void) 
{
	...
	if (kxtik1004_init_flag == -1)
	{
		GSE_ERR("kxtik1004_local_init, probe fail, return -1\n");
		return -1;
	}
	return 0;
}
...
//zhangxin 20150906 for accel choice
 	if ((zte_get_accelchipidx() !=ZTE_ACCEL_CHOICE_KXTIK1004)&&(zte_get_accelchipidx()!=ZTE_ACCEL_CHOICE_NODO))		
 	{
 		GSE_LOG("%s: accelidx=%d, not kxtik1004.\n", __func__, zte_get_accelchipidx());	
		return 0;
 	}
//zhangxin 20150906 for accel choice end
	...
	acc_driver_add(&kxtik1004_init_info);
	return 0;    
}
/*----------------------------------------------------------------------------*/
