...
#define LIS3DH_INIT_FAIL    (-1)
#define LIS3DH_INIT_SUCC    (0)
/*----------------------------------------------------------------------------*/
...
static int    s_nInitFlag = LIS3DH_INIT_FAIL;
...
//    	if(hwmsen_read_block(client, LIS3DH_REG_OUT_Y, &buf[2], 0x01))
    	if(hwmsen_read_byte_sr(client, LIS3DH_REG_OUT_Y, &buf[2]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
//    	if(hwmsen_read_block(client, LIS3DH_REG_OUT_Y+1, &buf[3], 0x01))
    	if(hwmsen_read_byte_sr(client, LIS3DH_REG_OUT_Y+1, &buf[3]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
...
//    	if(hwmsen_read_block(client, LIS3DH_REG_OUT_Z, &buf[4], 0x01))
    	if(hwmsen_read_byte_sr(client, LIS3DH_REG_OUT_Z, &buf[4]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
...
//    	if(hwmsen_read_block(client, LIS3DH_REG_OUT_Z+1, &buf[5], 0x01))
    	if(hwmsen_read_byte_sr(client, LIS3DH_REG_OUT_Z+1, &buf[5]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
...
static int LIS3DH_CheckDeviceID(struct i2c_client *client)
{
	u8 databuf[2]={0};    
	int res = 0;
	u8 addr = LIS3DH_REG_WHOAMI;

	struct lis3dh_i2c_data *obj = i2c_get_clientdata(client);

	if(hwmsen_read_byte_sr(client, addr, &databuf[0]))
	{
		GSE_ERR("LIS3DH_CheckDeviceID: read register err!\n");
		return LIS3DH_ERR_I2C;
	}
	
	//databuf[0] = LIS3DH_REG_WHOAMI;    
	GSE_ERR("LIS3DH_CheckDeviceID: deviceid=0x%x(default=0x%x)\n", databuf[0], LIS3DH_FIXED_DEVID);
...
static int lis3dh_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		GSE_ERR("lis3dh_device init failed\n");
...
static int  lis3dh_local_init(void)
{
    ...
    if(LIS3DH_INIT_FAIL == s_nInitFlag)
    {
        return -1;
    }
