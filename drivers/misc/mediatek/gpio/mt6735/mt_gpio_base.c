...
int mt_set_gpio_pull_enable_base(unsigned long pin, unsigned long enable)
{
    ...
    //printk("mt_set_gpio_pull_enable_base(%ld)(%ld)\n",pin,enable);
    ...
    if(PULLEN_offset[pin].offset == -1){
		...
		else
		{
			...
			//printk("qilin mt_set_gpio_pull_enable_base GPIO_PULL_EN_UNSUPPORTED(%lx)\n",pin);
			return GPIO_PULL_EN_UNSUPPORTED;
		}
    }
    else{
		
		//printk("qilin mt_set_gpio_pull_enable_base(%lx)(%lx)\n",(1L << (PULLEN_offset[pin].offset)),PULLEN_addr[pin].addr + 8);
