...
static ssize_t store_front_flashlight(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    U32 val = 0;
     if(buf != NULL && size != 0)
    {
     //   PMICLOG("[store_front_flashlight] buf is %s and size is %zu \n",buf,size);
        val = simple_strtoul(buf,&pvalue,2);
		if(val==0)
			{

		 mt_set_gpio_mode(GPIO_CAMERA_FLASH_EXT2_PIN,GPIO_MODE_00);
            mt_set_gpio_dir(GPIO_CAMERA_FLASH_EXT2_PIN,GPIO_DIR_OUT);
            mt_set_gpio_out(GPIO_CAMERA_FLASH_EXT2_PIN,GPIO_OUT_ZERO);
           gpio1_status=mt_get_gpio_out(GPIO_CAMERA_FLASH_EXT2_PIN);
...
extern  kal_uint8 my_set_sensor_mclk=240;
static ssize_t show_sensor_mclk(struct device *dev,struct device_attribute *attr,
					char *buf)
{
  return sprintf(buf, "%u\n", my_set_sensor_mclk);
}
static ssize_t store_sensor_mclk(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    char *pvalue = NULL;
    U32 val = 0;
     if(buf != NULL && size != 0)
    {
		...
		if(val!=0)
			{
			my_set_sensor_mclk= val;
			}
}
	 return size;
}
static DEVICE_ATTR(sensor_mclk, 0664, show_sensor_mclk, store_sensor_mclk);
/****************************************************************************
 * driver functions
 ***************************************************************************/
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		/*llt add*/
		rc = device_create_file(&(pdev->dev), &dev_attr_front_flashlight);
		rc = device_create_file(&(pdev->dev), &dev_attr_sensor_mclk);
		if (ret)
			goto err;
