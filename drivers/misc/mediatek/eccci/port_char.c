    ...
    case CCCI_IOC_SET_MD_SBP_CFG:
{
        ...
        sbp_custom_value="";
		   ...
		   printk("ydq,test1\n");
                              if(sbp_custom_value == "")
                              {
                                             CCCI_INF_MSG(md->index, CHAR, "kernel config not define SBP value, set it to 0!\n");
                                             md->sbp_code = 0;
											   printk("ydq,test2\n");
                              }
                              else
                              {
                                             ret = kstrtouint(sbp_custom_value, 0, &md->sbp_code_default);
            if (!ret){
               CCCI_INF_MSG(md->index, CHAR, "kernel SBP config:%d \n", md->sbp_code_default);
			   printk("ydq,kernel SBP config:%d \n", md->sbp_code_default);
                                                md->sbp_code = md->sbp_code_default;
            } else {
                   printk("ydq,kernel SBP config is invalid, value is %s\n", sbp_custom_value);
               CCCI_INF_MSG(md->index, CHAR, "kernel SBP config is invalid, value is %s\n", sbp_custom_value);
            }                                
                              }
                              /*
