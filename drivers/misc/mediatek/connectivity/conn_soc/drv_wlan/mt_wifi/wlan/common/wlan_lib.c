    ...
    )
{
    ...
    while(prQueueEntry) {
        ...
        if(eFrameAction == FRAME_ACTION_DROP_PKT) {
			...
			DBGLOG(INIT, WARN, ("send CMD Status:0x%x, Type:%d, CID:0x%x, Seq:%d\n",
    ...
    )
{
    ...
    do {
        ...
        if (!prCmdInfo->fgDriverDomainMCR) {
            ...
            if ((rStatus = nicTxAcquireResource(prAdapter, ucTC, pfgIsSecOrMgmt)) == WLAN_STATUS_RESOURCES) {
    ...
    )
{
    ...
    switch(prCmdInfo->eCmdType) {
	    ...
	    case COMMAND_TYPE_NETWORK_IOCTL:
			...
			DBGLOG(INIT, WARN, ("release Pending CMD Type:%d, CID:0x%x, Seq:%d\n",
    ...
    )
{
    ...
    if(prRegInfo->fg2G4BandEdgePwrUsed) {
        ...
        rCmdEdgeTxPwrLimit.cBandEdgeCert
            = getBandEdgeCert(prAdapter);
