...
static INT32 _stp_btm_handler(MTKSTP_BTM_T *stp_btm, P_STP_BTM_OP pStpOp)
{
    ...
    switch(pStpOp->opId) 
    {
		...
		dump_num = wmt_plat_get_dump_info(p_emi_ctrl_state_info->p_emi_ctrl_state_offset->emi_apmem_ctrl_chip_page_dump_num);
		if (dump_num != 0) {
				packet_num = dump_num;
		...
		do{
            ...
            if (dump_len <= 32*1024) {
				STP_BTM_INFO_FUNC("g_coredump_mode: %d!\n",g_coredump_mode);
				if (2 == g_coredump_mode)
					ret = stp_dbg_aee_send(&g_paged_dump_buffer[0],dump_len, 0);
				else if(1 == g_coredump_mode)
