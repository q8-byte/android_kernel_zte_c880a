...
static struct delayed_work fan5405_otg_work;
#endif 

#ifdef CONFIG_MTK_MAX77819_SUPPORT
extern int max77819_pmic_reg_update_bits(kal_uint8 reg, kal_uint8 mask, kal_uint8 val);

void max_usb_vbus_power_off(void)
{
	max77819_pmic_reg_update_bits(0x3D, 0x1<<0, 0x0);
	max77819_pmic_reg_update_bits(0x36, 0x1<<5, 0x0);
}

void max_usb_vbus_power_on(void)
{
	max77819_pmic_reg_update_bits(0x43, 0x7F<<0, 0x50);//MAX77819_RBOOST_CTL2		0x43
	max77819_pmic_reg_update_bits(0x3D, 0x1<<0, 0x1);//MAX77819_RBOOST_CTL1	0x3D
	max77819_pmic_reg_update_bits(0x36, 0x1<<5, 0x20);//MAX77819_BAT2SOC_CTL		0x36
}
#endif 
...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
    ...
    #elif defined(CONFIG_MTK_MAX77819_SUPPORT)
        printk("max77819_pmic power on vbus for otg \n");
        max_usb_vbus_power_on();
    #else
    ...
    #elif defined(CONFIG_MTK_MAX77819_SUPPORT)
        printk("max77819_pmic power off vbus for otg \n");
        max_usb_vbus_power_off();
    #else
        mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,GPIO_OTG_DRVVBUS_PIN_M_GPIO);
        mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN,GPIO_OUT_ZERO);
    #endif
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	if (mtk_musb->is_host) {
		...
		#ifdef CONFIG_MTK_FAN5405_SUPPORT
		schedule_delayed_work(&fan5405_otg_work,0);
		#endif 
		...
		#ifdef CONFIG_MTK_FAN5405_SUPPORT
		cancel_delayed_work(&fan5405_otg_work);
		#endif 
...
void fan5405_otg_rst_timer_work(void)
{
	/*T32s timer reset for boost mode*/
	fan5405_set_tmr_rst(1);
	schedule_delayed_work(&fan5405_otg_work,5*HZ);
}
#endif 
...
void mt_usb_otg_init(struct musb *musb)
{
	...
	#ifdef CONFIG_MTK_FAN5405_SUPPORT
	INIT_DELAYED_WORK(&fan5405_otg_work, fan5405_otg_rst_timer_work);
	#endif 
