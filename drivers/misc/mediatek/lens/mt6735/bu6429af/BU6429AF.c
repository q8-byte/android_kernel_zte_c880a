...
//resonant_frequency, a_current, b_current 
...
//	char puSendCmd[2];
	...
	resonant_frequency = 75; // 50 ~ 150 HZ
	a_current = 100; // 0 ~ 1023 LSB
	b_current = 140; // 0 ~ 1023 LSB

	//following 3 variables can be modified
	slewrate = 3; // 0, 1, 2, 3
	str = 4; // 1~7  LSB
	stt = 4; // 1~31 50 micro second
	...
	char puSendCmd1[2] = {(char)((MSB) & (Wx << 3)), (char)((rf << 3) & (slewrate))};
	...
	//step2: set point A current
	Wx = 0x2;
	
	char puSendCmd2[2] = {(char)((MSB) & (Wx << 3) & (a_current >> 8)), (char)(a_current)};
	...
	char puSendCmd3[2] = {(char)((MSB) & (Wx << 3) & (b_current >> 8)), (char)(b_current)};
	...
	//step4: set step-up code & step-up period
	Wx = 0x4;

	char puSendCmd4[2] = {(char)((MSB) & (Wx << 3)), (char)((str << 5) &(stt))};
