...
unsigned int msdc1_eint_flag = 0;
...
static int proc_Sdcard_Status_show(struct seq_file *m, void *v)
{
          kal_uint32 Tcard_Status = mt_get_gpio_in(GPIO_SDHC_EINT_PIN);	
		  
	          if(Tcard_Status==0)
			 Tcard_Status=1;	
		else if (Tcard_Status==1)
			 Tcard_Status=0;
	seq_printf(m, "%d\n",Tcard_Status);
...
static int proc_Sdcard_Status_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_Sdcard_Status_show, NULL);
}

static const struct file_operations Sdcard_Status_proc_fops = {
	.open = proc_Sdcard_Status_open,
	.read = seq_read,
...
{
	struct mmc_host *mmc;
   struct proc_dir_entry *Sdcard_Status_dir = NULL;
	struct msdc_host *host;
...
#endif
Sdcard_Status_dir = proc_mkdir("sdcard", NULL);
	if (!Sdcard_Status_dir) {
		pr_err("[%s]: mkdir /proc/Sdcard_Status failed\n", __func__);
	} else {

proc_create("sdcard_status", S_IRUGO | S_IWUSR, Sdcard_Status_dir, &Sdcard_Status_proc_fops);
		}
	host = mmc_priv(mmc);
	host->hw             = hw;
...
static int msdc_drv_shutdown(struct platform_device *pdev)
{
	struct mmc_host *mmc;
	struct msdc_host *host;
	...
	if(strcmp(pdev->dev.of_node->name, "MSDC1") == 0)
	{
		printk("start msdc_drv_shutdown %s\n",pdev->dev.of_node->name);
		msdc_deinit_hw(host);
	}
	return 0;
}
