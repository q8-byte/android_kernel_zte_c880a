...
int tpd_get_chipvendor_from_bootloader(struct i2c_client *i2c_client, int *vendorid_in_chip)
{
	...
	int *fwver_in_chip, int *vendorid_in_chip)
{
	...
	while (i < 2) {
		...
		*vendorid_in_chip = uc_reg_value;
		...
		if(0x00 == *vendorid_in_chip || 0xff == *vendorid_in_chip || FTS_REG_VENDOR_ID == *vendorid_in_chip) {
			printk("tpd [FTS] vendor id read abnormal, reset and read again. \n");
#ifdef GET_VENDORID_FROM_BOOTLOADER
			tpd_get_chipvendor_from_bootloader(i2c_client, &vendor_id);
	...
	if(0x00 == *vendorid_in_chip || 0xff == *vendorid_in_chip || FTS_REG_VENDOR_ID == *vendorid_in_chip) {
		printk("tpd [FTS] %s read firmware version is abnormal.", __func__);
		*vendorid_in_chip = 0x67;
		*fwver_in_chip = 0x00;
	}
#endif
	return 0;
...
int tpd_fts_ctpm_firmware_upgrade(struct i2c_client *client, unsigned char * pbt_buf, unsigned int size, unsigned int force_upg)
{
	...
	tpd_get_chipfw_version_etc(client, &chipid_in_chip, &fwver_in_chip, &vendorid_in_chip);
	
	fwver_in_file = pbt_buf[size - 2];
	vendorid_in_file = pbt_buf[size - 1];

	uc_tp_fm_ver = fwver_in_file;
	uc_host_fm_ver = fwver_in_chip;
	
	if(force_upg || ((vendorid_in_chip == vendorid_in_file || vendorid_in_chip == FTS_REG_VENDOR_ID) && 
		(fwver_in_chip == FTS_REG_FW_VER || fwver_in_chip < fwver_in_file)) )
	{
		printk("tpd [FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
		...
		printk("tpd [FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
			uc_tp_fm_ver, uc_host_fm_ver);
		printk("tpd [FTS] Not need upgrade firmware\n");
		i_ret = 0xff;
	}
	g_force_upgrade = 0;
	return i_ret;
...
static int fts_debug_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
...
static int fts_debug_read(struct file *file,
